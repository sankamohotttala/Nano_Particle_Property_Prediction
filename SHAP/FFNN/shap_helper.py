...
def shap_figure_change(label_fontsize=26, tick_fontsize=28, pad=0.02, fraction=0.025):
    cbar = plt.colorbar(fraction=fraction, pad=pad, aspect=40)
    cbar.ax.tick_params(labelsize=0, length=0)  # Hide ticks
    cbar.set_ticks([])
    cbar.set_label('Feature Value', fontsize=label_fontsize, labelpad=20)

    # Add 'Low' and 'High' labels at the start and end
    cbar.ax.text(0.5, -0.05, 'Low', ha='center', va='top', fontsize=tick_fontsize, transform=cbar.ax.transAxes)
    cbar.ax.text(0.5, 1.05, 'High', ha='center', va='bottom', fontsize=tick_fontsize, transform=cbar.ax.transAxes)
