...
from shap_helper import col_names, dataframes_shap,residu_best_worst,residu_calc,shap_figure_change
import matplotlib
...
#         self.fc1 = nn.Linear(input_size, 64)
        ...
        # self.final_act_new = SinActivation_
        self.final_act_new = nn.Identity()
        ...
        self.activ_list=[{'leaky_relu':nn.LeakyReLU(), 'relu': nn.ReLU(), 'gelu':nn.GELU(), 'sigmoid':nn.Sigmoid(), 'tanh':nn.Tanh()} for i in range(len(self.net_core)-1)]
    ...
    def forward(self, x):
        ...
        for i in range(len(self.net_core)-1):
            ...
            x = self.net_core[i](x)
            # x = self.final_act(x)
            x = self.activ_list[i][self.activation](x)
        x = self.net_core[-1](x) #last layer has no activations
        # x = self.relu(x) 
        # x = self.final_act_new(x)
        return x
...
predict_values_dic = {'train':None, 'val':None, 'test':None} #do not add train set since shuffling isdone in train
actual_values_dic = {'train':None, 'val':None, 'test':None}
...
for trial , dataset_loader in trial_dict.items():
    ...
    if trial != 'train':
        predict_values_dic[trial] = predicted_values
        actual_values_dic[trial] = actual_values
...
#______________SHAP____________________

# features_train
# model.train()
# torch.set_grad_enabled(True)
train_feature_500_ = shap.utils.sample(features_train, 200) 
train_feature_500_ = train_feature_500_.astype('float32')
train_feature_500 = torch.from_numpy(train_feature_500_)
# train_feature_500.to(torch.float32)

LR_lasso_model = model
explainer = shap.DeepExplainer(LR_lasso_model, train_feature_500)
# explainer = shap.Explainer(LR_lasso_model, train_feature_500_)

# shap_values = explainer.shap_values(features_test_t)
shap_values = explainer.shap_values(features_test_t[:,:])
# shap_values = explainer(features_test_t[:10,:])
plt.close('all')

col_np, cols = col_names()

features_train_df, features_test_df, shap_values_df, shap_values_abs_df ,shap_values_abs_df_describe_sorted = dataframes_shap(shap_values, features_test,features_train, cols)


num_features = features_test_df.shape[1]
# shap.summary_plot(shap_values, features_test_t[:10,:],max_display=num_features) #without actual feature names
# Increase DPI and save SHAP plots

# Save SHAP summary plot (dot)

plt.figure(figsize=(12, 12))
shap.summary_plot(shap_values, features_test_df.iloc[:,:], max_display=35, show=False, color_bar=False)
plt.xlabel('SHAP value', fontsize=26)
plt.ylabel('Feature Name (Descending Order)', fontsize=26)
plt.xticks(fontsize=24)
plt.yticks(fontsize=24)
shap_figure_change() # adds a new color bar to the plot
plt.savefig(os.path.join(output_folder, 'shap_summary_dot.png'), dpi=600, bbox_inches='tight')
plt.close()

# Save SHAP summary plot (bar)
plt.figure(figsize=(12, 12))
shap.summary_plot(shap_values, features_test_df.iloc[:,:], plot_type="bar", max_display=8, show=False)
# plt.gcf().set_dpi(600)
plt.savefig(os.path.join(output_folder, 'shap_summary_bar.png'), dpi=600, bbox_inches='tight')
plt.close()


test_residu_np, test_residu_np_abs = residu_calc(actual_values_dic,predict_values_dic)
best_10, worst_10, best_set_ind, worst_set_ind = residu_best_worst(test_residu_np,test_residu_np_abs,shap_values)


best_feature_10_df = pd.DataFrame(features_test[best_set_ind], columns=cols)
worst_feature_10_df = pd.DataFrame(features_test[worst_set_ind], columns=cols)


plt.figure()
shap.summary_plot(best_10, best_feature_10_df , max_display=8, show=False, color_bar=False) # best 10%
plt.xlabel('SHAP value', fontsize=20)
plt.ylabel('Feature Name', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
shap_figure_change(label_fontsize=20, tick_fontsize=20, pad=0.1, fraction=0.035) # adds a new color bar to the plot
plt.savefig(os.path.join(output_folder, 'best_10_dot.png'), dpi=600, bbox_inches='tight')
plt.close()

plt.figure()
shap.summary_plot(worst_10, worst_feature_10_df , max_display=8, show=False, color_bar=False) # worst 10%
plt.xlabel('SHAP value', fontsize=20)
plt.ylabel('Feature Name', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
shap_figure_change(label_fontsize=20, tick_fontsize=20, pad=0.1, fraction=0.035) # adds a new color bar to the plot
plt.savefig(os.path.join(output_folder, 'worst_10_dot.png'), dpi=600, bbox_inches='tight')
plt.close()
