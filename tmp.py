#code taken from 2024_ICARC - just to check with 10-fold code
import pandas as pd
import numpy as np
...
from utility_FFNN import saveFile , MAPD_percentage, create_CSV_for_outputs, device_select, plot_confusion_matrix,save_classification_report
...
args, parser, folder_path = arg_parser() # cmd > default args

device = device_select(args.device_manual) #currently set to cpu
...
csv_df, csv_path = create_CSV_for_outputs(output_path) # iitializing the dataframe for csv or loading the previously used csv as a dataframe
                                                        # since we are not using the architecture type, csv doesn't have that column 'architecture_type'
...
input_num_features = args.input_num_features #63
output_size = args.output_num_features #3

#define hyper-parameter
batch_size_ = args.batch_size #original 100
init_weights = args.init_weights # 'x_n'
lr_val = args.lr_val # 0.0001
num_epochs = args.num_epochs #original 100 --- use this value
...
num_hidden_layers = args.num_hidden_layers  # 5
nodes_per_layer = args.nodes_per_layer #[ 128, 64, 32,16,8] 
...
features__ = args.processed_features        #'normalized_35' # 'normalized_35' , 'raw_63'
labels__ = args.label_type      #'neg_one_zero_pos_one' # 'r_avg_raw' , 'n_bulk_raw', 'n_surface_raw'
...
file_path_features = os.path.join(path_datasets , 'FS.csv')
file_path_normalized_features = os.path.join(path_datasets , 'WBF.csv')
file_path_standardize_features = os.path.join(path_datasets , 'WOBF.csv')

features_FS = pd.read_csv(file_path_features)
features_WBF = pd.read_csv(file_path_normalized_features)
features_WOBF = pd.read_csv(file_path_standardize_features)

# data_train_X_WBF = features_WBF.loc[:, lambda x: [l for l in features_WBF if l != "Class"]]
label_FS= features_FS.loc[:,'Class']
label_WBF = features_WBF.loc[:,'Class']
label_WOBF = features_WOBF.loc[:,'Class'] 

# data_train_Y_WBF = features_WBF.loc[:, lambda x: "Class"] 
features_FS.drop(columns=['Class'], inplace=True)
features_WBF.drop(columns=['Class'], inplace=True)
features_WOBF.drop(columns=['Class'], inplace=True)



labels_Class = label_FS # since all the above labels are the same!


# if standardize_features == True:
#       # print(standardize_cols[standardize])
#       scaler = StandardScaler()
#       # scaler = MinMaxScaler()

#     #   scale_cols = standardize_cols[standardize]
#       # print(scale_cols)

#       scaler.fit(features_FS)
#       data_train_X_tmp = scaler.transform()
#       data_test_X_standard = scaler.transform(data_test_X[scale_cols])

#       for ind , col in enumerate(scale_cols):
#           data_test_X[col] = data_test_X_standard[:,ind]
#           data_train_X[col] = data_train_X_tmp[:,ind]
...
print(features_WBF.shape)
print(features_WOBF.shape)
print(features_FS.shape)
print(labels_Class.shape)

feature_WBF_names = list(features_WBF.columns)
feature_FS_names = list(features_FS.columns)
feature_WOBF_names = list(features_WOBF.columns)

# ## Visualize data
if histogram_visualize:
    data = features_WBF.to_numpy()
    # plt.figure(figsize=(20, 6))
    fig, axs = plt.subplots(nrows=1, ncols=63, figsize=(140, 10))
    for i in range(63):
        axs[i].hist(data[:, i], bins=30, color='blue', alpha=0.7)
        axs[i].set_title(feature_WBF_names[i])
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'WBF_63_features.png'))
    if show_figures:
        plt.show()
    plt.close()

    data = features_FS.to_numpy()
    # plt.figure(figsize=(20, 6))
    fig, axs = plt.subplots(nrows=1, ncols=54, figsize=(140, 10))
    for i in range(54):
        axs[i].hist(data[:, i], bins=30, color='blue', alpha=0.7)
        axs[i].set_title(feature_FS_names[i])
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'FS_54_featues.png'))
    if show_figures:
        plt.show()
    plt.close()

    data = features_WOBF.to_numpy()
    # plt.figure(figsize=(10, 6))
    fig, axs = plt.subplots(nrows=1, ncols=59, figsize=(70, 10))
    for i in range(59):
        axs[i].hist(data[:, i], bins=30, color='blue', alpha=0.7)
        axs[i].set_title(feature_WOBF_names[i])
    plt.tight_layout()
    plt.savefig(os.path.join(output_folder, 'WOBF_59_features.png'))
    if show_figures:
        plt.show()
    plt.close()

    data = labels_Class.to_numpy()
    plt.figure(figsize=(10, 6))
    plt.hist(data[:], bins=30, color='blue', alpha=0.7)
    plt.title('label 1')
    plt.xlabel('Value')
    plt.ylabel('Frequency')
    plt.savefig(os.path.join(output_folder, 'class_.png'))
    if show_figures:
        plt.show()
    plt.close()
...
features = features_WBF

if features__ == 'WBF_63':
    features = features
elif features__ == 'WOBF_59':
    features = features_WOBF 
elif features__ == 'FS_54':
    features = features_FS
else:
    ValueError('not a valid feature set!')

if labels__ == 'neg_one_zero_pos_one':
    labels = labels_Class
else:
...
features_train, features_test, labels_train, labels_test = train_test_split(features_np, labels_np, test_size=0.3, random_state=42)
...
features_validation, features_test, labels_validation, labels_test = train_test_split(features_test, labels_test, test_size=0.1, random_state=42)
...
if standardize_features == True:
    ...
    data_train_X_tmp = scaler.transform(features_train)
    data_test_X_standard = scaler.transform(features_validation)

    #   for ind , col in enumerate(scale_cols):
    #       data_test_X[col] = data_test_X_standard[:,ind]
    #       data_train_X[col] = data_train_X_tmp[:,ind]

    features_train = data_train_X_tmp
    features_validation = data_test_X_standard
...
class FFNN_non_linear(nn.Module):
    def __init__(self, input_size = 63, output_size =3, init_weights = 'x_u', n_hidd_layers = None, nodes = None, activation = 'relu'):
...
#         self.fc1 = nn.Linear(input_size, 64)
    ...
    def forward(self, x):
        ...
        # x = self.Softmax(x) #final activation probabilities
        return x
...
optimizer = optim.Adam(model.parameters(), lr=lr_val, weight_decay=0.001)
...
def weight_plot(weights, weight_names, initial =False, epoch = -1):
    ...
    for i in range(num_plots):
        c_val = i % 3
        r_val = i // 3
        if weights[i].cpu().detach().numpy().flatten().sum()==0:
            continue
...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        actual_val_train.extend([int(val.cpu().numpy()) for val in labels])
        preddictd_probabilities = outputs.softmax(dim=1)
        predicted_val_train.extend([int(val.cpu().detach().numpy()) for val in  torch.argmax(preddictd_probabilities,dim =1)])
        

        

    accuracy_train = accuracy_score(actual_val_train, predicted_val_train)
    report_train = classification_report(actual_val_train, predicted_val_train)
    save_classification_report(report_train, output_folder, epoch,train ='train')
    conf_matrix_train = confusion_matrix(actual_val_train, predicted_val_train)
    train_acc.append(accuracy_train)
    if epoch % 100 == 0:
        plot_confusion_matrix(conf_matrix_train,output_folder,epoch, show_figures,train ='train')

    # lin_conf_total = np.round(lin_conf_total)

    # lin_conf_total += lin_conf_matrix    
    ...
    with torch.no_grad():
        for inputs, labels in val_loader:
            ...
            actual_val_valida.extend([int(val.cpu().numpy()) for val in labels])
            preddictd_probabilities_valida = outputs.softmax(dim=1)
            predicted_val_valida.extend([int(val.cpu().detach().numpy()) for val in  torch.argmax(preddictd_probabilities_valida,dim =1)])



    accuracy_valida = accuracy_score(actual_val_valida, predicted_val_valida)
    report_valida = classification_report(actual_val_valida, predicted_val_valida)
    save_classification_report(report_valida, output_folder, epoch,train ='validation')
    conf_matrix_valida = confusion_matrix(actual_val_valida, predicted_val_valida)
    valid_acc.append(accuracy_valida)
    if epoch % 100 == 0:
        plot_confusion_matrix(conf_matrix_valida,output_folder,epoch, show_figures,train ='validation')
...
#     learning_rates.append(scheduler.get_last_lr()[0])  # store the learning rate
    ...
    print(f'accuracy_train: {accuracy_train:.6f}')
    print(f'accuracy_valida: {accuracy_valida:.6f}')
...
#     scheduler.step()
    ...
    if epoch % 100000 == 0: #originallly 10
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(output_folder, 'train_val_acc.png'))
if show_figures:
    plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(output_folder, 'val_acc.png'))
if show_figures:
    plt.show()
...
plt.legend()
plt.grid(True)
plt.savefig(os.path.join(output_folder, 'train_acc.png'))
if show_figures:
    plt.show()
